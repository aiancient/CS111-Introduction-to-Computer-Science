...
public class ScenarioAnalysis {
    ...
    /*
     * Computes and updates the CO2 emissions, fuel cost(entire length of lease) 
     * and total cost(fuel & other cost) for each 
    ...
    public void computeCO2EmissionsAndCost () {
        ...
        {
            int carType = vehicles[i].getFuel().getType(); 

            if(carType==1) 
            {
                //Gas
                int miles = vehicles[i].getLease().getMileageAllowance(); 
                double fuelEfficienty = vehicles[i].getFuel().getUsage(); 
                double num = Fuel.CO2EMITTED_GASCOMBUSTION; 
                double answer = (miles/fuelEfficienty) * (num); 
                vehicles[i].setCO2Emission(answer);
            ...
            else if(carType==2)
            {
                //electric
                int miles = vehicles[i].getLease().getMileageAllowance(); 
                double drivingRangeOnCharge = vehicles[i].getFuel().getUsage(); 
                double batterySC = vehicles[i].getFuel().getKWhPerCharge(); 
                double num = Fuel.CO2EMITTED_GENERATEmWh; 
                double answer = ((miles/drivingRangeOnCharge)*batterySC) * ((num*(0.001))*0.45); 
                vehicles[i].setCO2Emission(answer);
            }

            int milesPerMonth = vehicles[i].getLease().getMileageAllowance() / 12; 
            int totalMiles = milesPerMonth * (vehicles[i].getLease().getNumberOfMonths()); 
            vehicles[i].setFuelCost((double) totalMiles * gasPrice);

            double leaseCost = vehicles[i].getLease().getDueAtSigning() + (vehicles[i].getLease().getMonthlyCost()) * (vehicles[i].getLease().getNumberOfMonths()); 
            double total = vehicles[i].getOtherCost() + vehicles[i].getFuelCost() + leaseCost; 
